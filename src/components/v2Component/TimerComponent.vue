...
const timeIsUp = ref(" ");
const emit = defineEmits(["sendTimeOver", "getCurrentSec"]);
const props = defineProps({
  endBeforeTime: Boolean,
});
...
watch(
  () => props.endBeforeTime,
  (newValue) => {
    console.log("props.endBeforeTime", props.endBeforeTime);
    clearInterval(intervalId);
    emit("getCurrentSec", currentSecound.value);
  }
);
</script>
